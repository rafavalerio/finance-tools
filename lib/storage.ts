import { MortgageInputs, Expense, BuyerType, RepaymentFrequency, ExpenseFrequency } from "@/types/mortgage";
...
export function encodeMortgageData(data: MortgageStorageData): string {
  try {
    ...
    if (inputs.interestRate !== DEFAULTS.interestRate) compact[KEY_MAP.interestRate] = inputs.interestRate;
    if (inputs.loanTermYears !== DEFAULTS.loanTermYears) compact[KEY_MAP.loanTermYears] = inputs.loanTermYears;
    ...
    if (inputs.offsetBalance !== DEFAULTS.offsetBalance) compact[KEY_MAP.offsetBalance] = inputs.offsetBalance;
